...
Provides a reusable `<Select>` component styled like the existing Combobox but without a search input. The Select replaces native `<select>` elements throughout the app with a consistent, keyboard-accessible dropdown. Collision detection, overflow protection, and independent dropdown width requirements also apply to the existing `Combobox` component — both share the `useDropdownPosition` hook.
...
#### Scenario: Uses absolute positioning within the nearest positioned ancestor
- **WHEN** the dropdown is open
- **THEN** it SHALL use `position: absolute` with `top-full` (default) or `bottom-full` (when flipped)
- **BECAUSE** absolute positioning lets the dropdown scroll naturally with the page and avoids scroll-position tracking complexity

#### Scenario: Uses high z-index to render above overlapping content
- **WHEN** the dropdown is open
- **THEN** it SHALL use `z-50` (Tailwind, equivalent to `z-index: 50`)

#### Scenario: Uses useLayoutEffect to prevent flash of incorrect position
- **WHEN** the dropdown opens
- **THEN** the collision detection SHALL run in `useLayoutEffect` (not `useEffect`) so the flip state is computed before the browser paints the dropdown
- **BECAUSE** `useEffect` fires after paint, causing a flash where the dropdown briefly renders below the viewport before flipping upward

#### Scenario: Recalculates position on each open
- **WHEN** the dropdown opens
- **THEN** the component SHALL call `trigger.getBoundingClientRect()` to compute the current viewport position
