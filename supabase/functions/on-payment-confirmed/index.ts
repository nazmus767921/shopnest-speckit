...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("[Webhook on-payment-confirmed] Received payload type:", payload.type)

    // Trigger condition: table update on payment_confirmations where confirmed_at transitions from null to non-null
    const isTargetUpdate = 
      payload.type === "UPDATE" &&
      payload.table === "payment_confirmations" &&
      payload.record &&
      payload.record.confirmed_at !== null &&
      (!payload.old_record || payload.old_record.confirmed_at === null)

    if (!isTargetUpdate) {
      console.log("[Webhook on-payment-confirmed] Trigger conditions not met, ignoring.")
    ...
    console.log(`[Webhook on-payment-confirmed] Processing confirmed payment for order: ${orderId}, merchant: ${merchantId}`)

    // Initialize Supabase client inside Deno
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!
    const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!
    const supabase = createClient(supabaseUrl, supabaseKey)

    // 1. Fetch Order and Merchant details
    ...
    if (orderError || !order) {
      console.error("[Webhook on-payment-confirmed] Error fetching order:", orderError)
    ...
    if (merchantError || !merchant) {
      console.error("[Webhook on-payment-confirmed] Error fetching merchant:", merchantError)
    ...
    if (itemsError || !items) {
      console.error("[Webhook on-payment-confirmed] Error fetching order items:", itemsError)
    ...
    console.log(`[Webhook on-payment-confirmed] Email recipient resolved: ${customerEmail}`)

    // 4. Construct Order Items HTML list
    let orderItemsHtml = ""
    items.forEach((item) => {
      const priceText = "৳" + (item.unit_price_paisa / 100).toFixed(2)
    ...
    // 5. Generate Email HTML Template (Matching DESIGN.md aesthetic)
    const emailHtml = `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <title>Order Confirmed - ${merchant.name}</title>
        <style>
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <p style="color: #52525b; font-size: 14px; line-height: 1.5; margin-bottom: 24px;">
              Your payment has been successfully verified by the merchant! Your order is now **Processing** and being packed for shipping.
            </p>
    ...
    const emailResult = await sendEmail({
      to: customerEmail,
      subject: `Your order is confirmed! — ${merchant.name}`,
      html: emailHtml,
    ...
    console.error("[Webhook on-payment-confirmed] Fatal error:", errorMsg)
