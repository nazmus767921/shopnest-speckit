...
export function SubmitPaymentForm({ usageCounts, currentPlan, plans, preselectedPlanSlug }: SubmitPaymentFormProps) {
  ...
  // Styled Mock QR Code SVG depending on payment method
  const renderMockQRCode = (method: "bkash" | "nagad") => {
    const brandColor = method === "bkash" ? "#e2136e" : "#f15a22"
    return (
      <div className="relative flex flex-col items-center justify-center p-4 bg-white border border-hairline-light rounded-xl w-36 h-36 mx-auto select-none">
        <svg
          viewBox="0 0 100 100"
          className="w-28 h-28"
          fill="none"
        ...
        <span
          className="absolute bottom-1 text-[9px] font-bold uppercase tracking-widest"
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex items-center justify-between border-b border-hairline-light pb-4">
        <div className="flex items-center gap-1.5">
          <div className="p-1.5 bg-aloe-10 rounded-lg text-ink">
            <Wallet className="h-4.5 w-4.5" />
          </div>
          <div>
            <h3 className="text-body-strong font-bold text-ink">Renewal &amp; Upgrade Portal</h3>
            <p className="text-micro text-shade-50">Manage subscription payments manually</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              className={`w-6.5 h-6.5 rounded-full flex items-center justify-center text-micro font-bold transition-all ${step === s
                  ? "bg-primary text-on-primary"
                  : step > s
                    ? "bg-aloe-10 text-ink"
                    : "bg-zinc-100 text-shade-40 border border-hairline-light"
                }`}
            >
      ...
      {error && (
        <div className="text-caption text-red-600 bg-red-50/50 border border-red-200 p-3.5 rounded-xl flex items-start gap-2">
          <TriangleAlert className="w-4 h-4 shrink-0 mt-0.5" />
          <span>{error}</span>
        </div>
      )}
      {success && (
        <div className="text-caption text-emerald-600 bg-emerald-50/50 border border-emerald-200 p-3.5 rounded-xl flex items-start gap-2">
      ...
      {step === 1 && (
        <form.Subscribe selector={(state) => state.values.targetPlan}>
          {(targetPlan) => {
            ...
            return (
              <div className="text-caption text-red-600 bg-red-50 border border-red-200 p-3.5 rounded-xl flex flex-col gap-1">
      ...
      >
                    ...
                    {activePlans.map((p) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative p-5 border rounded-xl flex flex-col gap-2 transition-all select-none w-full ${planLocked
                              ? "border-red-100 bg-red-50/10 opacity-60 cursor-not-allowed"
                              : isSelected
                                ? "border-primary bg-zinc-50/40 ring-1 ring-primary cursor-pointer"
                                : "border-hairline-light hover:border-shade-40 bg-canvas-light cursor-pointer"
                            }`}
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col">
                              <span className="text-body-strong font-semibold text-ink flex items-center gap-1.5">
                                {p.name}
                                {planLocked && (
                                  <span className="text-red-600 text-micro flex items-center gap-1 font-bold">
                            ...
                            {isActivePlan && (
                              <span className="text-[10px] bg-emerald-50 text-emerald-800 border border-emerald-100 px-2 py-0.5 rounded-full font-semibold uppercase tracking-wider">
                                Active
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-baseline gap-0.5 mt-2">
                            <span className="text-heading-xl font-bold font-display text-ink">৳{(p.pricePaisa / 100).toLocaleString("en-BD")}</span>
                            <span className="text-micro text-shade-50">/ month</span>
                          </div>

                          <ul className="text-caption text-shade-50 space-y-1 pt-2 border-t border-hairline-light leading-normal">
                          ...
                          {planLocked && (
                            <div className="text-micro text-red-600 bg-red-50/50 border border-red-200/60 p-2.5 rounded-lg flex flex-col gap-1 mt-2">
              ...
              {(field) => (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div
                    ...
                    className={`p-3.5 border rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer select-none text-center ${field.state.value === "bkash"
                        ? "border-[#e2136e] bg-[#e2136e]/5 ring-1 ring-[#e2136e] text-[#e2136e]"
                        : "border-hairline-light hover:border-shade-40 bg-canvas-light text-ink"
                      }`}
                  >
                    <span className="text-body-strong font-bold">bKash</span>
                  ...
                  <div
                    ...
                    className={`p-3.5 border rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer select-none text-center ${field.state.value === "nagad"
                        ? "border-[#f15a22] bg-[#f15a22]/5 ring-1 ring-[#f15a22] text-[#f15a22]"
                        : "border-hairline-light hover:border-shade-40 bg-canvas-light text-ink"
                      }`}
                  >
                    <span className="text-body-strong font-bold">Nagad</span>
                    <span className="text-[10px] opacity-75">Send Money (Personal)</span>
                  </div>
                </div>
              )}
              ...
              {({ paymentMethod, targetPlan }) => {
                ...
                return (
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="border border-hairline-light rounded-xl p-4 bg-canvas-cream/20 flex flex-col gap-4">
                      {renderMockQRCode(paymentMethod)}

                      <div className="flex flex-col gap-2 pt-2 border-t border-hairline-light">
                        <div className="flex justify-between items-center text-caption">
                          <span className="text-shade-50">Transfer Amount:</span>
                          <span className="font-bold text-ink text-body-strong">{amount}</span>
                        </div>
                        <div className="flex justify-between items-center text-caption">
                          <span className="text-shade-50">Wallet Number:</span>
                          <div className="flex items-center gap-1.5">
                            <span className="font-mono font-bold text-ink bg-canvas-light border border-hairline-light px-2 py-0.5 rounded text-micro">
                              {number}
                            </span>
                            <button
                              type="button"
                              onClick={() => copyToClipboard(number.replace("-", ""))}
                              className="p-1 hover:bg-black/5 rounded transition-all text-shade-50 hover:text-ink cursor-pointer"
                    ...
                    <div className="text-micro text-shade-50 bg-zinc-50 border border-hairline-light p-3 rounded-lg leading-normal flex items-start gap-1.5">
                      <QrCode className="w-4 h-4 shrink-0 text-shade-50 mt-0.5" />
                      <span>
          ...
          <div className="flex flex-col gap-5 animate-fade-in">
            ...
            <form.Subscribe selector={(state) => state.values}>
              {({ paymentMethod, targetPlan }) => {
                ...
                const amount = targetPlanObj ? "৳" + (targetPlanObj.pricePaisa / 100).toLocaleString("en-BD") : "—"
                return (
                  <div className="text-caption text-shade-60 bg-zinc-50 border border-hairline-light p-3.5 rounded-xl flex flex-col gap-1 leading-normal">
                    <span className="font-semibold text-ink">Selected transaction parameters:</span>
                    <span>Plan: <strong className="capitalize">{targetPlanObj?.name || targetPlan}</strong></span>
                    <span>Channel: <strong className="capitalize">{paymentMethod}</strong></span>
                ...
                return (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      ...
                      error={hasError}
                      className="font-mono uppercase tracking-wider text-body-strong rounded-xl bg-canvas-light py-3"
                    />
                    {hasError ? (
                      <span className="text-micro text-red-600 flex items-center gap-1 font-medium mt-0.5">
        ...
        <span className="text-[10px] text-shade-40 text-center leading-normal flex items-center justify-center gap-1 pt-2 border-t border-hairline-light">
          <TriangleAlert className="w-3.5 h-3.5 text-shade-40 shrink-0" />
