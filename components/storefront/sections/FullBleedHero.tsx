    ...
    <div className="relative w-[100vw] min-h-[90vh] left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] flex flex-col lg:flex-row bg-[var(--color-background,white)] overflow-hidden">
      ...
      <div className="flex-[0.4] flex flex-col justify-center px-8 md:px-16 lg:px-24 py-20 lg:py-0 bg-transparent z-10">
        <div className="max-w-lg space-y-8">
          <h1 className="text-6xl md:text-7xl lg:text-[5.5rem] font-serif italic text-ink leading-[1.05] tracking-tight animate-fade-in-up">
            {title}
          </h1>
          
          {subtitle && (
            <p className="text-lg md:text-xl text-ink/80 font-sans font-light animate-fade-in-up" style={{ animationDelay: "150ms" }}>
              {subtitle}
            </p>
          )}
          ...
          {buttonText && (
            <div className="pt-8 animate-fade-in-up" style={{ animationDelay: "300ms" }}>
              <Link href={buttonLink || "/products"}>
                <Button 
                  size="lg" 
                  className="rounded-[var(--radius)] bg-primary! text-on-primary! hover:opacity-80 border-none font-sans font-normal px-12 py-7 text-xs uppercase tracking-[0.2em] transition-opacity duration-500"
                >
                  {buttonText}
                </Button>
              </Link>
      ...
      <div className="flex-[0.6] relative min-h-[50vh] lg:min-h-full">
        {imageUrl ? (
          ...
          <div className="absolute inset-0 w-full h-full bg-shade-30 flex items-center justify-center">
            <span className="text-shade-50 font-sans font-light tracking-widest uppercase text-sm">Hero Image Placeholder</span>
          </div>
        )}
      </div>
