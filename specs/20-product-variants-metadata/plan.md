...
**UX Model (per Clarifications 2026-07-03 + 2026-07-04)**: Shopify-style inline table with chip option inputs. No wizard, no modals. Attributes added via inline "+" button; option values typed as chips/pills (Enter to add, × to remove). Variants populate instantly. SKU/price/stock cells click-to-edit inline. Bulk toolbar + search/filter bar. Smart merge on attribute changes: new options add variants, removed options **cascade-delete** affected variants (with merchant warning), attribute deletion cascade-deletes all variants. Active carts referencing deleted variants show "No longer available" and must be removed before checkout. Product auto-reverts to non-variant mode when last attribute deleted.
...
- **Cascade-delete constraint**: When an attribute option is removed, associated variants are cascade-deleted (with a warning dialog showing affected count). When an entire attribute is deleted, all its variants are cascade-deleted (with a warning dialog). The DB enforces this via ON DELETE CASCADE foreign keys. Smart merge preserves existing price/SKU/stock overrides on surviving variants and adds new variants for new options. Manual variant toggle (is_active) remains available for merchant control — cascade-delete only triggers on attribute/option removal.
...
|-----------|-----------|--------|
| **I. Library-First** | Variant matrix computation (Cartesian product), smart merge algorithm, and variant selection logic MUST be extracted to standalone libraries (`lib/products/variants.ts`, `lib/products/attributes.ts`). No inline matrix generation in page components. Smart merge is a pure algorithm — given old attributes + new attributes + existing variants, it returns diff (add, delete, preserve). | ✅ PASS |
...
| **V. Invariants** | Invariant 2 (stock non-negative) extends to variant stock — must use Postgres transaction guard. Invariant 3 (price snapshotted) applies to variant price. Invariant 1 (merchant_id) applies to all variant queries. Invariant 8 (UI primitives) applies to variant selectors and variant table components. Invariant 9 (DESIGN.md First) applies to inline variant table, chip inputs, bulk toolbar, and filter bar. **Cascade-delete rule (new invariant)**: Attribute deletion cascade-deletes options, variant_attribute_links rows, and variants via ON DELETE CASCADE at the DB level. No orphaned data. Cart items referencing deleted variants are shown as "No longer available" and block checkout. | ✅ PASS |
