...
- **Tokens**: `{spacing.0.5}` (2px) · `{spacing.1}` (4px) · `{spacing.2}` (8px) · `{spacing.3}` (12px) · `{spacing.4}` (16px) · `{spacing.6}` (24px) · `{spacing.8}` (32px) · `{spacing.16}` (64px).
- **Section padding**: `{spacing.16}` (64px) to `32` (128px) on cinematic marketing pages (extreme negative space is the point); collapses to `12` (~48px) on transactional pages where density takes priority.
- **Card internal padding**: `{spacing.8}` (32px) on pricing cards; `{spacing.6}` (24px) on compact tag rows.
...
## No Shadows Rule
The application enforces a strict flat design aesthetic. No drop shadows, box shadows, or inset shadows are allowed anywhere in the project. Depth is achieved purely through contrasting canvas colors, high-quality merchant photography, and clear hairline borders (`{colors.hairline-light}` / `{colors.hairline-dark}`).
...
- Background `{colors.primary}` (black), text `{colors.on-primary}`, type `{typography.body-md}`, padding `{spacing.3} {spacing.6}` (12px 24px), rounded `{rounded.pill}` 9999px.
...
- Background `{colors.canvas-night-elevated}`, text `{colors.on-primary}`, rounded `{rounded.lg}`. Holds full-bleed photography or a single large statement.
...
- Background `{colors.canvas-light}`, text `{colors.ink}`, type `{typography.body-md}`, padding 10px `{spacing.3}` (12px), rounded `{rounded.md}` 8px, 1px `{colors.hairline-light}` border.

### Navigation

**`nav-bar-light`** — top nav on light pages.
- Background `{colors.canvas-light}`, text `{colors.ink}`, padding `{spacing.4} {spacing.6}`. Logo wordmark on the left, nav items center, two pill buttons on the right (`button-outline-on-light` for "Log in", `button-primary-pill` for "Start free trial").
...
- Background `{colors.aloe-10}`, text `{colors.ink}`, type `{typography.eyebrow-cap}`, padding `{spacing.1} {spacing.3}`, rounded `{rounded.pill}`.
...
**No-Shadow Styling** — the entire design system uses zero drop shadows or gradients of shadow. Depth and card boundaries are established strictly with hairlines (`{colors.hairline-light}` / `{colors.hairline-dark}`) and canvas color shifts.
...
- Background `{colors.canvas-night}`, text `{colors.on-primary}`, type `{typography.caption}`, padding `{spacing.16} {spacing.6}`. Contains 4–5 columns of muted-tone link groups, social icons, and a small legal row.
...
- Don't add drop shadows, box shadows, or inset shadows anywhere in the layout; the design wants flat blackness or pure cream/mint with no shadow fuzziness.
