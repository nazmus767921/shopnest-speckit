---
version: alpha
name: ShopNest-Fashion-Template
description: The Fashion Boutique template — an editorial, magazine-inspired storefront designed for clothing, accessories, shoes, and fashion-forward brands. Thin-weight Playfair Display serif headings at large sizes create an editorial luxury feel, paired with Inter for UI body text. The design alternates between a warm off-white canvas (#FAF9F6) for shopping surfaces and a rich near-black (#111111) for the footer and newsletter bands, creating cinematic contrast. Lookbook-style product presentation, prominent color swatches, minimal navigation, and generous whitespace define the aesthetic. No shadows — depth comes from canvas contrast and photography.
...
The Fashion template is a magazine-editorial storefront that makes products feel aspirational. Where the General template shouts with bold black uppercase headlines, Fashion whispers with elegant Playfair Display serifs at thin weights — the kind of typography you'd see on Vogue, ELLE, or high-end boutique e-commerce like SSENSE or Net-a-Porter.

The canvas is warm off-white (`#FAF9F6`) — not clinical white, but a paper-like warmth that makes product photography feel editorial. The footer and newsletter bands flip to a rich near-black (`#111111`) for cinematic contrast, creating a high-fashion duality between light shopping surfaces and dark statement sections.

Product presentation is lookbook-style: tall 3:4 portrait cards that emphasize model and lifestyle shots over product-on-white shots. Color swatches appear as small circles with ring indicators. Size selection uses pills that invert to black-on-white when active. Everything is designed to make browsing feel like flipping through a curated fashion magazine.

Navigation is deliberately sparse — the fashion template strips away category mega-menus and dense search bars in favor of a minimal top bar that lets the content breathe. "Shop", "New In", and a search icon are all that appears. The search opens as a full-width overlay rather than an inline bar.
...
- **Mixed-case display headings** — unlike the general template, fashion display headings are NOT uppercase. They use title case or sentence case for an editorial, approachable feel.
...
- **Cart**: Shared implementation with fashion template tokens.
...
### Grid
- Product grid: 3 columns desktop, 2 columns tablet, 2 columns mobile (fashion keeps 2-col on mobile for visual density).
- Card gap: `{spacing.lg}` (24px).
- Max content width: 1200px (slightly narrower than general for more editorial feel).

### Spacing Philosophy
The fashion template uses **generous whitespace** — sections breathe with `{spacing.section}` (80px) to `{spacing.section-lg}` (120px) between content bands. The hero section uses extreme vertical padding. This whitespace is the brand — it signals luxury and intentionality, in contrast to the general template's tighter, more efficient spacing.
...
- **`btn-outline`** — Transparent pill with ink border. Hover inverts to filled black with white text (a fashion-specific interaction — the outline "fills in" on hover).
...
- Active: **Inverted** — `{colors.primary}` background, `{colors.on-primary}` text. This is a fashion-template signature interaction.

### Navigation (Fashion Navbar)
...
- Use portrait 3:4 aspect ratio for product cards — the tall format is the fashion template's visual identity.
- Use generous whitespace between sections (80–120px) — the breathing room signals luxury.
- Use the inverted size pill (black fill + white text on active) — it's a fashion-template signature.
...
- Don't add add-to-cart buttons on product cards — the fashion template encourages browsing to PDP.
...
- Don't use square product cards — the 3:4 portrait format is the fashion template's identity.
...
| Mobile | < 768px | 2-col portrait grid (fashion keeps 2-col for visual density), stacked PDP, compact hero with centered text, hamburger nav |
