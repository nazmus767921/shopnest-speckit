...
export function FashionProductCard({ product, subdomain, merchantId }: FashionProductCardProps) {
  ...
  return (
    <div className="group flex flex-col gap-3.5 select-none text-left">
      {/* Tall aspect-[3/4] Image Container */}
      <Link
        href={`/product/${product.slug}`}
        className="relative aspect-[3/4] w-full overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-hairline-warm)] bg-[var(--color-surface-product)] block"
      >
        {imageUrl ? (
          <Image
            src={imageUrl}
      ...
      <div className="flex flex-col gap-1.5 px-0.5">
            ...
            <div className="flex items-center gap-1">
              {swatches.map((opt) => (
                <span
                  ...
                  style={{
                    backgroundColor: opt.swatchColor || "#cccccc"
                  }}
        ...
        <div className="flex items-center justify-between gap-4 mt-2 border-t border-[var(--color-hairline-warm)]/40 pt-3">
          ...
          <button
            ...
            className="h-8 px-4 rounded-full bg-black hover:bg-zinc-800 text-white font-sans text-[10px] font-bold tracking-wider uppercase transition-colors cursor-pointer select-none disabled:opacity-40 disabled:cursor-not-allowed shrink-0 border-none"
            aria-label={
              isOutOfStock
                ? "Out of stock"
                : hasVariants
